...
const CustomTableBody = <D extends object>({ rows, columns, getTableBodyProps, prepareRow, loading, counterSTT, data }: TableBodyIProps<D>) => {
    return (
        <TableBody {...getTableBodyProps()}>
            {!loading &&
                rows.map((row, i) => {
                    let dataRow: any = row.original;
                    let style: React.CSSProperties = dataRow["style"] ? (dataRow["style"] as React.CSSProperties) : ({} as React.CSSProperties);
                    prepareRow(row);
                    return (
                        <TableRow style={style} {...row.getRowProps()}>
                            <Cell scope="row" align="center" style={{ border: "1px solid #F5F5F5", padding: "4px 8px" }}>
                                {i + 1 + counterSTT()}
                            </Cell>
            ...
            {loading && (
                <TableRow>
                    <Cell scope="row" colSpan={columns.length + 2} style={{ border: "1px solid #F5F5F5", textAlign: "center" }}>
                        <LinearProgress />
                    </Cell>
                </TableRow>
            )}
            {!loading && rows.length === 0 && (
                <TableRow>
                    <Cell colSpan={columns.length + 2} style={{ border: "1px solid #F5F5F5", textAlign: "center" }}>
                        Không có dữ liệu
                    </Cell>
                </TableRow>
            )}
